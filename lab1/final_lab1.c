...
/*long double getUniform(
  ...
  printf("How many random numbers do you want to print? \n");
  scanf("%d", &num);
  int n = num;
  ...
  int i;
  for(i=0; i<n; i++) {
  	double result = rand()/(double)RAND_MAX*100;
  	printf("%lf\n",result);
...
long int points(long double angle, long double radius, long double r_angle, long double r_radius) {
  ...
  // long double ang = (long double)(angle%360.0);
...
void location(long double angle, long double radius, long double skill, int num) {
   srand(time(NULL));
   isSpareReady = false;
